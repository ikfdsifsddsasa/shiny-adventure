local CONFIG = {
	VALID_KEY = "BFRU-PREM-7Kx9",
	TRAP_KEY = "BFRU-FREE-0kLm",
...
local ACCENT = Color3.fromRGB(255, 90, 50)
local ACCENT_DARK = Color3.fromRGB(170, 45, 25)
...
local COLORS = {
	...
	stroke = Color3.fromRGB(215, 70, 30),
...
local function buildAuthGui()
	...
	authGui.Name = "BloxFruitsDupAuth"
	...
	subtitle.Text = "Blox Fruits — enter license key"
...
buildMainGui = function()
	...
	Sub.Text = "Blox Fruits | RightShift"
	...
	statusLbl.Text = "Ready — Dup Fruits placeholder"
	...
	addLabel(dupTab, "— FRUIT DUP —")
	addTextBox(dupTab, "Fruit name...", "")
	addButton(dupTab, "Dup Fruit", noop)
	addButton(dupTab, "Dup Selected", noop)
	addButton(dupTab, "Dup x10", noop)
	addButton(dupTab, "Dup All Fruits", noop)
	addLabel(dupTab, "— INVENTORY —")
	addButton(dupTab, "Refresh Inventory", noop)

	scrollCounter = 0
	local devTab = tabFrames.Dev
	addLabel(devTab, "— STUDIO —")
	addButton(devTab, "Print PlaceId", function()
		print("[Dup Fruits] PlaceId:", game.PlaceId)
		updateStatus("PlaceId: " .. tostring(game.PlaceId))
	end)
	addButton(devTab, "Print Player", function()
		print("[Dup Fruits] Player:", LocalPlayer.Name)
	...
	print("[Dup Fruits] Menu ready | RightShift = toggle")
end
